<template>
  <div id="main">
    <el-card>
      ...
      <template v-if="allTodoList">
        <ul>
          <li
            v-for="(item, index) in allTodoList"
            :key="index"
            class="text item"
          >
            <el-checkbox
              ...
              class="check"
              :class="item.hidden ? 'hiddenItem' : ''"
              @change="completedItem(index)"
            ></el-checkbox>
            <label
              @dblclick="editTodo(index)"
              :class="[
                item.completed ? 'labelCompleted' : 'labelInit',
                item.hidden ? 'hiddenItem' : '',
              ]"
              >{{ item.content }}</label
            >
            <input
              ...
              class="editInput"
              :class="item.hidden ? 'displayInput' : 'hiddenItem'"
              @blur="saveTodo(index)"
              @keyup.enter="saveTodo(index)"
              @keyup.esc="saveTodo(index)"
            />
            <el-button
              icon="el-icon-close"
        ...
        <div class="footer">
          <el-row>
            <el-col :span="9"
              ><span
                ><strong>{{
                  this.allTodoList.filter((item) => !item.completed).length
                }}</strong>
                items left</span
              ></el-col
            >
            ...
            >
            <el-col :span="3"
              ><el-button plain size="small" @click.stop="handleGoto('active')"
                >Active</el-button
              ></el-col
            >
            <el-col :span="4"
              ><el-button
                plain
                size="small"
                @click.stop="handleGoto('completed')"
                >Completed</el-button
              ></el-col
            >
            <el-col :span="6"
              ><el-button
                plain
                size="small"
                v-if="this.allTodoList.some((element) => element.completed)"
...
export default {
  ...
  methods: {
    ...
    getActive() {
      let temp = JSON.parse(localStorage.getItem("allTodoList"));
      this.allTodoList = temp.filter((item) => !item.completed);
    },
    getCompleted() {
      let temp = JSON.parse(localStorage.getItem("allTodoList"));
      this.allTodoList = temp.filter((item) => item.completed);
